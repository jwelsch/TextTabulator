...
namespace TextTabulator.Adapters.ReflectionTests
{
    public class ReflectionTabulatorAdapterTests
    {
        ...
        [Fact]
        public void When_called_with_single_item_and_with_axes_orientation_swapped_then_data_is_returned_with_rows_and_columns_swapped()
        {
            var item = new TestClass2()
            {
                StringProperty = "Hello",
            ...
            var options = new ReflectionTabulatorAdapterOptions(null, TypeMembers.Properties, AccessModifiers.Public, null, AxesOrientation.Swapped);
            var sut = new ReflectionTabulatorAdapter<TestClass2>(item, options);
            ...
            Assert.Collection(values, new Action<IEnumerable<string>>[]
            {
                i => Assert.Collection(i, new Action<string>[]
                {
                    j => Assert.Equal(nameof(TestClass2.StringProperty), j),
                    j => Assert.Equal(item.StringProperty, j),
                }),
                i => Assert.Collection(i, new Action<string>[]
                {
                    j => Assert.Equal(nameof(TestClass2.IntProperty), j),
                    j => Assert.Equal(item.IntProperty.ToString(), j),
                }),
                i => Assert.Collection(i, new Action<string>[]
                {
                    j => Assert.Equal(nameof(TestClass2.EnumerableProperty), j),
                    j => Assert.Equal(item.EnumerableProperty.ToString(), j),
                }),
            });
        }
        ...
        [Fact]
        public void When_called_with_multiple_items_and_with_axes_orientation_swapped_then_data_is_returned_with_rows_and_columns_swapped()
        {
            var items = new TestClass2[]
            {
                new()
            ...
            {
                i => Assert.Equal("Member", i),
                i => Assert.Equal("Value0", i),
                i => Assert.Equal("Value1", i),
                i => Assert.Equal("Value2", i),
            });
            Assert.Collection(values, new Action<IEnumerable<string>>[]
            {
                i => Assert.Collection(i, new Action<string>[]
        ...
        [Fact]
        public void When_called_with_columns_sorted_ascending_and_axes_swapped_then_data_is_returned_with_rows_and_columns_swapped_and_in_correct_order()
        {
            var items = new TestClass2[]
            {
                new()
